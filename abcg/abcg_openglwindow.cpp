...
GLuint abcg::OpenGLWindow::createProgramFromFile(
    std::string_view pathToVertexShader,
    std::string_view pathToFragmentShader, std::string_view pathToGeometryShader) {
  std::stringstream vertexShaderSource;
  if (std::ifstream stream(pathToVertexShader.data()); stream) {
    vertexShaderSource << stream.rdbuf();
    stream.close();
  ...
  if (std::ifstream stream(pathToGeometryShader.data()); stream) {
    geometryShaderSource << stream.rdbuf();
    stream.close();
  } else {
    throw abcg::Exception{abcg::Exception::Runtime(fmt::format(
        "Failed to read fragment shader file {}", pathToGeometryShader))};
  }

  return createProgramFromString(vertexShaderSource.str(),
                                 fragmentShaderSource.str(), geometryShaderSource.str());
}


GLuint abcg::OpenGLWindow::createProgramFromString(
    std::string_view vertexShaderSource,
    std::string_view fragmentShaderSource, std::string_view geometryShaderSource) {
  using namespace std::string_literals;
...
std::string geoSource{abcg::trimCopy(std::string{geometryShaderSource})};
#if defined(__EMSCRIPTEN__) || defined(__APPLE__)
  // Remove version header, if any
  fsSource =
      std::regex_replace(geoSource, std::regex("#version.*[\r\n|\n|\r]"), "");

  if (m_openGLSettings.profile == OpenGLProfile::ES) {
    // c++20
    // if (auto regex{std::regex("^precision.*float$", std::multiline)};
    if (auto regex{std::regex("(^|\r\n|\n|\r)precision.*float")};
        !std::regex_search(geoSource, regex)) {
      geoSource = "precision mediump float;\n" + geoSource;
    }
  }
  // Add new header
  geoSource = m_GLSLVersion + "\n" + geoSource;
#else
  // Add version header only if missing
  if (!geoSource.starts_with("#version")) {
    if (m_openGLSettings.profile == OpenGLProfile::ES)
      geoSource = "precision mediump float;\n\n" + geoSource;
    geoSource = m_GLSLVersion + "\n\n" + geoSource;
  }
  ...
  const char *geoSourceConstChar = geoSource.c_str();
  glShaderSource(geometryShader, 1, &geoSourceConstChar, nullptr);
  glCompileShader(geometryShader);
  glGetShaderiv(geometryShader, GL_COMPILE_STATUS, &compileStatus);
  if (compileStatus == 0) {
    printShaderInfoLog(geometryShader, "Geometry shader");
    glDeleteShader(vertexShader);
    glDeleteShader(fragmentShader);
    throw abcg::Exception{
        abcg::Exception::Runtime("Failed to compile fragment shader")};
